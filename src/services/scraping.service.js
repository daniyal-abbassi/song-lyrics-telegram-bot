...
async function _scrapeFromBingPage(page) {
  ...
  try {
    ...
    if (lyricsArray.length > 0) {
      ...
      logger.info("Successfully scraped lyrics directly from Bing search results.");
      return lyrics;
    }
    logger.debug("Bing lyrics container found, but no lyric verses inside.");
    return null;
  } catch (error) {
    logger.debug("No embedded lyrics found on Bing page. Moving to next strategy.");
    return null;
  }
}
...
async function _findLyricsSourceUrl(page) {
    logger.debug("Searching for Musixmatch or LyricsTranslate URL in search results.");
...
async function _scrapeFromMusixmatch(page, url) {
    ...
    try {
        await page.goto(url, { waitUntil: "networkidle2", timeout: config.puppeteer.navigationTimeout });
        ...
        const lyrics = await page.evaluate(() => {
            const lyricsElements = Array.from(document.querySelectorAll('div.css-146c3p1.r-1inkyih.r-11rrj2j.r-13awgt0.r-fdjqy7.r-1dxmaum.r-1it3c9n.r-135wba7[dir="auto"]'));
...
async function _scrapeFromLyricsTranslate(page, url) {
    ...
    try {
        ...
        logger.warn("Navigated to LyricsTranslate, but no lyrics content was found.");
        return null;
    } catch (error) {
        logger.error({ error: error.message }, "Failed to scrape lyrics from LyricsTranslate.");
        return null;
    }
}
...
async function getLyrics(songName, artistName) {
  let browser;
  logger.debug({ songName, artistName }, "Starting multi-strategy scraping process.");
  ...
  try {
    ...
    logger.warn(`All scraping strategies failed for "${songName} by ${artistName}".`);
    throw new LyricsNotFoundError(`Could not find lyrics for "${songName} by ${artistName}" after trying multiple sources.`);

  } catch (error) {
    logger.error({ error: error.message, stack: error.stack }, "An error occurred during the scraping process.");
